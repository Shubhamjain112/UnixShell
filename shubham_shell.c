...
#include <string.h>
#include <sys/types.h>
#include <unistd.h>	//This contains chdir(), fork(), exec(), pid_t, pipe()
#include <sys/stat.h>
#include <fcntl.h>
#include <signal.h>
...
void spawn_proc (int in, int out, struct commandList *cmd)
{
	...
	if ((pid = fork ()) == 0)
	{
		if (in != 0)
  		{
			dup2 (in, 0);//Here dup2 is duplicate
			close (in);
  		}
		if (out != 1)
		{
...
void fork_pipes (int n, struct commandList *cmd)
{
	if(strcmp(cmd[0].argument_array[0]	, "cd")==0)
	{
		...
		{
			if(chdir(cmd[0].argument_array[1])!=0)	//If the path is invalid then it returns non zero value.
			{
				printf("Invalid Path\nType Correctly :P \n");
			}
		}
	...
	{	//Creat a new child process here.
		pid_t pid;
  		int i;
  		int ini, out;
		int  status;
...
{
	char *token;
	token = strtok(cmd, " ");	//Breaks string into number of strings on the basis of space.
	while( token != NULL )
   	{
   	   ...
   	   if(token)
   	   {
			...
			if(strcmp(token, "&") == 0)
			{
				...
				argument_count = 0;	//HEre argument_count is set to zero!
				pipeing++;
			}
			...
			if(strcmp(token, "|") == 0)
			{
				command[pipeing].argument_array[argument_count] = NULL;
				command[pipeing].argument_count = argument_count;
				argument_count = 0;//Here Argument count is set to zero as we will have new query here.
				pipeing++;
			}
		}
